...
@foreach($data as $d)
 <form action="{{ route('mahasantri.update',$d->id) }}" method="POST">
    ...
    <div class="form-group">
        <label>nama</label><input type="text" name="nama" valeu="{{ $d->nama }}" class="form-control"/>
    </div>
    <div class="form-group">
        <label>nim</label><input type="text" name="nim" valeu="{{ $d->nim }}" class="form-control"/>
    </div>
    ...
    <div class="form-group">
        ...
        <option value="">-- Pilih dosen--</option>
            @foreach($rs1 as $dsn)
            @php
                $sel1 = ($dsn->id == $d->dosen_id) ? 'selected' : ''; 
            @endphp
    ...
    <div class="form-group">
        ...
        <option value="">-- Pilih matakuliah--</option>
            @foreach($rs3 as $mk)
            @php
                $sel3 = ($mk->id == $mk->matakuliah_id) ? 'selected' : ''; 
            @endphp
